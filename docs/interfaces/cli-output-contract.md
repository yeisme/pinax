...
- Default output: concise Chinese summary; on success, show only result facts, risks, and next steps, and do not show command execution status.
...
- New notebook core commands must reuse the same projection: default output for `daily`, `inbox`, `view`, `index`, `search`, `note links/backlinks/orphans/attach/attachments`, `import markdown`, `export markdown`, `organize plan/list/apply`, `project board show/plan/configure/export`, `project item add/move/archive`, and `api routes/schema/serve` is a Chinese summary, `--json` is a single envelope, and `--agent` is low-token facts.
...
- Argument errors under `--json`, `--agent`, `--events`, and `--explain` must be rendered from the same failed projection, containing stable `error.code`, machine-safe `error.message`, and executable `error.hint` / `actions`.
...
- `pinax note links`, `pinax note backlinks`, `pinax note orphans`, and `pinax search --link-target` must render the default Chinese summary, `--agent`, `--json`, `--events`, and `--explain` from the same relationship projection.
