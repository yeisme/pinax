...
本地候选实现，默认关闭。能力资源：`pinax://input/capabilities`；入口：`pinax.input.prepare`；用途：`markdown / attachment`；项目绑定：已存在的 --vault。完成回执：pinax://input/<id>；需 preview/apply 后由原导入或附件服务写入。
...
```bash
pinax mcp serve --vault ./vault --input-listen 127.0.0.1:8740 --input-base-url http://127.0.0.1:8740
```
...
实现 change：`openspec/changes/pinax-mcp-input-intake-v1/`。本地 fixture 证据保存在 `temp/integration-test-runs/`；发布、远端部署与真实 Provider 验证单独执行。不能用已连接旧二进制的发现结果推断本地候选代码已部署。
