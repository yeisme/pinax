...
- Capsa Sync should treat the memory ledger as local service-owned memory evidence, not as plaintext cross-device content. Source notes and receipts remain the portable authority; any future cross-device memory sync needs an explicit encrypted contract instead of uploading raw ledger state.
