...
func TestScorerRanksFieldSignalsAboveBodyFallback(t *testing.T) {
	...
	hits := scorer.Score(RecallFilter{Query: "release"}, []Candidate{
		{Record: scorerRecord("body", TypeFact, "notes", "plain", "release appears only in the body", now.Add(-time.Hour))},
		{Record: scorerRecord("predicate", TypeFact, "release_workflow", "tag push", "", now.Add(-2*time.Hour))},
	}, nil, 10)
	if len(hits) != 2 {
...
func TestScorerSourceConfidenceFreshnessAndTaskFitness(t *testing.T) {
	...
	hits := scorer.Score(RecallFilter{Query: "release test"}, []Candidate{
		{Record: scorerRecord("file", TypeDecision, "release_file", "workflow", "", now.Add(-2*time.Hour))},
		{Record: func() Record {
			r := scorerRecord("openspec", TypeDecision, "release_decision", "workflow", "", now.Add(-24*time.Hour))
			r.SourceURI = "openspec/changes/release/design.md"
			r.Confidence = "high"
			return r
		}()},
		{Record: func() Record {
			r := scorerRecord("event", TypeEvent, "release_event", "test run passed", "", now.Add(-time.Hour))
...
func TestScorerTieBreakAndCollapse(t *testing.T) {
	now := time.Date(2026, 6, 24, 8, 0, 0, 0, time.UTC)
	scorer := Scorer{Now: now}
	old := scorerRecord("old", TypeFact, "release", "old", "", now.Add(-3*time.Hour))
	newer := scorerRecord("newer", TypeFact, "release", "new", "", now.Add(-2*time.Hour))
	superseding := scorerRecord("superseding", TypeFact, "release", "newer", "", now.Add(-time.Hour))
	...
	a := scorerRecord("a", TypeFact, "cloud", "same", "", now.Add(-time.Hour))
	b := scorerRecord("b", TypeFact, "kb", "same", "", now.Add(-time.Hour))
...
func scorerRecord(id, typ, predicate, object, body string, created time.Time) Record {
	return Record{ID: id, Type: typ, Subject: "pinax", Predicate: predicate, Object: object, Body: body, Status: StatusConfirmed, Confidence: "confirmed", SourceURI: "file://memory.md", CreatedAt: created, UpdatedAt: created}
}
