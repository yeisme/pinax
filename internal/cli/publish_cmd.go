...
func addPublishCommands(root *cobra.Command, ctx commandBuildContext) {
	...
	publishCmd := &cobra.Command{
		...
		Example: "pinax publish profile init public --target github-pages --renderer hugo --vault ./my-notes --json\n" +
	...
	publishPlanCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			projection, err := ctx.svc.PublishPlan(cmd.Context(), app.PublishRequest{VaultPath: *ctx.vaultPath, Profile: profileName, Target: planTarget})
			return ctx.renderProjection(cmd, projection, err)
		},
	}
	publishPlanCmd.Flags().StringVar(&profileName, "profile", "", "Publish profile name")
	publishPlanCmd.Flags().StringVar(&planTarget, "target", "", "Override publish target: github-pages, github-wiki, github-gist, or http")
	...
	publishBuildCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			projection, err := ctx.svc.PublishBuild(cmd.Context(), app.PublishRequest{VaultPath: *ctx.vaultPath, Profile: profileName, Target: buildTarget, Out: buildOut})
			return ctx.renderProjection(cmd, projection, err)
		},
	}
	publishBuildCmd.Flags().StringVar(&profileName, "profile", "", "Publish profile name")
	publishBuildCmd.Flags().StringVar(&buildTarget, "target", "", "Override publish target: github-pages, github-wiki, github-gist, or http")
	...
	publishDoctorCmd.Flags().StringVar(&doctorTarget, "target", "", "Override publish target: github-pages, github-wiki, github-gist, or http")
	...
	publishServeCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			projection, err := ctx.svc.PublishServe(cmd.Context(), app.PublishRequest{VaultPath: *ctx.vaultPath, Profile: profileName, Out: serveOut, Host: serveHost, Port: servePort, Once: serveOnce})
			return ctx.renderProjection(cmd, projection, err)
		},
	}
	...
	profileInitCmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			projection, err := ctx.svc.PublishProfileInit(cmd.Context(), app.PublishRequest{VaultPath: *ctx.vaultPath, Profile: args[0], Target: profileTarget, Renderer: renderer, Title: title, BaseURL: baseURL, Theme: theme})
			return ctx.renderProjection(cmd, projection, err)
		},
	}
	profileInitCmd.Flags().StringVar(&profileTarget, "target", "github-pages", "Publish target: github-pages, github-wiki, github-gist, or http")
	profileInitCmd.Flags().StringVar(&renderer, "renderer", "hugo", "Renderer: hugo or none")
