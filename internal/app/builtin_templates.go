...
func builtInTemplates() map[string]string {
	templates := map[string]string{
		...
		"journal.daily":   strings.Join([]string{"---", "schema_version: pinax.template.v2", "kind: journal_template", "name: journal.daily", "title: Daily Note", "engine: go-template", "output:", "  path_pattern: daily/{{ .Date }}.md", "defaults:", "  kind: daily", "  status: active", "---", "# {{ .Date }}", "", "## Today's Focus", "", "- ", "", "## Notes", "", "- ", "", "## Daily Task Review", "<!-- pinax:managed name=daily-task-review -->", "", "<!-- /pinax:managed -->", "", "## Pinax Captures", "<!-- pinax:managed name=daily-captures -->", "", "<!-- /pinax:managed -->", "", "## Review", ""}, "\n"),
