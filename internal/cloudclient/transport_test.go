...
func TestServerTransportMapsCloudsyncOperations(t *testing.T) {
	...
	commit, err := transport.CommitRevision(ctx, cloudsync.CommitRequest{BaseRevision: "", RevisionID: "rev_b", ManifestBlobID: "manifest_a", BlobIDs: []string{"blob_a"}, DeviceID: "laptop", RequestID: "req_1"})
	if err != nil {
	...
	_, err = transport.CommitRevision(ctx, cloudsync.CommitRequest{BaseRevision: "", RevisionID: "rev_c", ManifestBlobID: "manifest_a", BlobIDs: []string{"blob_a"}, DeviceID: "laptop", RequestID: "req_2"})
	if err == nil {
...
func TestServerTransportNeverRemoteWriteBeforeCommit(t *testing.T) {
	...
	_, commitErr := transport.CommitRevision(ctx, cloudsync.CommitRequest{BaseRevision: "", ManifestBlobID: "manifest_missing", BlobIDs: []string{"blob_a"}, DeviceID: "laptop", RequestID: "req_gate"})
	if commitErr == nil {
	...
	commit, err := transport.CommitRevision(ctx, cloudsync.CommitRequest{BaseRevision: "", ManifestBlobID: "manifest_ok", BlobIDs: []string{"blob_a"}, DeviceID: "laptop", RequestID: "req_gate_ok"})
	if err != nil {
		t.Fatalf("commit: %v", err)
	}
