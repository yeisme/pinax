...
- [ ] 2.1 新增 GORM model 和 migration service。
  - Owner: Pinax
  - Lane: B
  - Depends on: 1.1
  - Scope: 新建 `internal/memory/model.go`、`internal/memory/store.go`，新增 `memory_records`、`memory_entities`、`memory_record_entities`、`memory_sources`，使用 GORM，不在业务层硬编码 SQL。
...
- [ ] 2.2 新增 FTS5 projection adapter。
  - Owner: Pinax
  - Lane: B
  - Depends on: 2.1
...
- [ ] 3.1 实现 `MemoryCapture`、`MemoryList`、`MemoryRecall`、`MemoryContext`、`MemoryStats` app service。
  - Owner: Pinax
  - Lane: C
  - Depends on: 2.1
...
- [ ] 4.1 新增 `pinax memory` 命令族。
  - Owner: Pinax
  - Lane: D
  - Depends on: 3.1
  - Scope: 新建 `internal/cli/memory_cmd.go` 并在 root command 注册；命令包括 `capture/list/recall/context/link/prune/stats` 的第一版可用切片。
