...
- [x] 1.1 写失败测试：workflow metadata 被解析但旧模板继续可用。
  - Owner: `cli/pinax`
  - Lane: A
  ...
  - Evidence: RED `go test ./internal/app -run 'TestTemplateWorkflowMetadata|TestBuiltinTemplates|TestTemplatePack|TestTemplateLifecycle' -count=1` 因 `Metadata.ScenarioID` 等 workflow 字段缺失失败；GREEN 同命令通过。

- [x] 1.2 实现 workflow metadata 解析和默认值。
  - Owner: `cli/pinax`
  - Lane: A
  ...
  - Evidence: `go test ./internal/app -run 'TestTemplateWorkflowMetadata|TestBuiltinTemplates|TestTemplatePack|TestTemplateLifecycle' -count=1` 通过；legacy `daily` 保留兼容默认值。

## 2. Intent recommendation 升级为 workflow recommendation

- [x] 2.1 写 CLI contract 失败测试：recommend 返回 workflow recommendation。
  - Owner: `cli/pinax`
  - Lane: B
  ...
  - Evidence: RED `go test ./cmd/pinax -run 'TestTemplateRecommend|TestTemplateInspect|TestTemplatePreview|TestTemplatePack|TestTemplateLifecycle' -count=1` 先暴露 recommendation scoring 抢主推荐，再暴露 `recommendation.0.*` agent key 缺失；GREEN 同命令通过。

- [x] 2.2 实现本地 metadata-only recommendation scoring。
  - Owner: `cli/pinax`
  - Lane: B
  ...
  - Evidence: `go test ./cmd/pinax -run 'TestTemplateRecommend|TestTemplateInspect|TestTemplatePreview|TestTemplatePack|TestTemplateLifecycle' -count=1` 通过；`template recommend` 保留 `data.primary`/`data.templates` 并追加 `data.recommendations[]`。

## 3. Inspect/preview 暴露 workflow starter 和写入影响

- [x] 3.1 写 inspect/preview 输出合同失败测试。
  - Owner: `cli/pinax`
  - Lane: C
  ...
  - Evidence: `TestTemplateInspectWorkflowFields` 和 `TestTemplatePreviewWorkflowFields` 覆盖 workflow/proof/output_policy/next_command；preview 测试对比文件列表验证只读。

- [x] 3.2 实现 inspect/preview projection。
  - Owner: `cli/pinax`
  - Lane: C
  ...
  - Evidence: `go test ./cmd/pinax -run 'TestTemplateRecommend|TestTemplateInspect|TestTemplatePreview|TestTemplatePack|TestTemplateLifecycle' -count=1` 通过。
...
- [x] 4.1 写模板创建使用证据失败测试。
  ...
  - Evidence: RED `go test ./cmd/pinax -run 'TestTemplateUseEvidence|TestNoteAddTemplate' -count=1` 因缺少 `template_pack`、`scenario_id`、`proof_gate.status` 和 `data.template_use` 失败；GREEN 同命令通过。

- [x] 4.2 实现 use evidence 和可选 receipt 写入。
  - Owner: `cli/pinax`
  - Lane: D
  ...
  - Evidence: `CreateNote` projection additive 输出 `template_use_id`、`template_pack`、`scenario_id`、`effective_path`、`proof_gate.status` 和 `data.template_use.next_actions[]`；service 写入 note event 摘要；`go test ./cmd/pinax -run 'TestTemplateUseEvidence|TestNoteAddTemplate' -count=1` 通过。

## 5. Local template pack 和 lifecycle 门禁

- [x] 5.1 写 pack/lifecycle 失败测试。
  - Owner: `cli/pinax`
  - Lane: E
  ...
  - Evidence: app 层 `TestTemplatePack`/`TestTemplateLifecycle` 和 CLI 层 `TestTemplatePackAndLifecycleCLI` 覆盖 starter pack、draft 非 executable primary、vault-local override 来源。

- [x] 5.2 实现 pack/lifecycle discovery。
  - Owner: `cli/pinax`
  - Lane: E
  ...
  - Evidence: `go test ./internal/app -run 'TestTemplateWorkflowMetadata|TestBuiltinTemplates|TestTemplatePack|TestTemplateLifecycle' -count=1` 和 `go test ./cmd/pinax -run 'TestTemplateRecommend|TestTemplateInspect|TestTemplatePreview|TestTemplatePack|TestTemplateLifecycle' -count=1` 通过；实现只读取 builtin 和 vault-local 模板。
...
- [x] 6.1 更新用户文档和本地开发说明。
  ...
  - Evidence: 更新 `docs/commands/template.md` 的 workflow catalog、template use evidence 和 scenario matrix；`rg -n 'workflow catalog|template recommend --intent|template_use_id|proof_gate|template pack' docs openspec/changes/pinax-template-workflow-catalog` 命中 docs 和 OpenSpec。

- [x] 6.2 补充 scenario matrix evidence。
  - Owner: `cli/pinax`
  - Lane: F
  ...
  - Evidence: `docs/commands/template.md` 增加 `capture-sticky`、`idea-research-seed`、`meeting-decision`、`learning-pack`、`stock-learning`、`index-page` 矩阵；`rg -n 'capture-sticky|idea-research-seed|meeting-decision|learning-pack|stock-learning|index-page' docs openspec/changes/pinax-template-workflow-catalog` 命中。
...
- [x] 7.1 OpenSpec 验证。
  ...
  - Evidence: `openspec validate pinax-template-workflow-catalog --strict` 通过；`openspec validate --all --strict` 通过，51 passed, 0 failed。
...
- [x] 7.2 代码质量门禁。
  ...
  - Evidence: `task check` 通过，覆盖 `golangci-lint fmt --diff`、`golangci-lint run`、`go test ./...`、`go build -trimpath -ldflags="-s -w" -o dist/pinax ./cmd/pinax`、`task kb:sidecar:protocol` 和 `openspec validate --all`。
