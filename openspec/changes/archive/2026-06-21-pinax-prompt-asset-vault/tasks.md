...
- 2026-06-21: refreshed integration evidence with `task test:integration`; current reproducible run is `temp/integration-test-runs/20260621T073517Z-965454/` and includes `summary.json`, `command.txt`, `stdout.log`, `stderr.log`, `env.json`, and `artifacts/`. Redaction scan for `Authorization: Bearer|Bearer [A-Za-z0-9._-]+|PLUGIN_RUNTIME_SECRET_SENTINEL|DATAVIEW_DATABASE_BODY_SENTINEL|SECRET_BODY_SENTINEL|raw prompt|provider payload|full chain-of-thought|hidden system prompt|private tool arguments|PRIVATE_SENTINEL_DO_NOT_PUBLISH` had no matches.
