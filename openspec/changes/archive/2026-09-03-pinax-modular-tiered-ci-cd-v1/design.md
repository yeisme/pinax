...
本 change 只拥有 `cli/pinax` 内的 workflow、质量命令、项目文档和验证，不把其他仓库作为完成门禁。
...
- Profile: `Go release CLI`.
- quick commands: `task check`
- full commands: `task ci`, `goreleaser check`
- integration commands: `task test:integration`
- release: Manual snapshot and SemVer tag publish use separate modules.
