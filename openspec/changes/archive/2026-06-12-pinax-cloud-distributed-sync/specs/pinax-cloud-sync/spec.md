...
- **AND** until that apply path is wired, CLI SHALL return a structured partial/error with `remote_write=false` rather than silently succeeding
...
- **AND** until the rclone adapter and lock-object commit protection are implemented, sync apply SHALL return a structured partial/error with `remote_write=false`
