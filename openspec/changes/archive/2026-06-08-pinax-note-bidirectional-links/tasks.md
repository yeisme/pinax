...
- [x] 3.8 增加增量与全量一致性 benchmark 和回归测试。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 新增 `internal/index/consistency_test.go`，覆盖 full rebuild 与 incremental changed/moved/deleted 事件序列的最终 note/link/search snapshot 一致性；运行 `go test ./internal/index ./internal/app -run 'IncrementalMatchesFullRebuild' -count=1`，退出码 0。新增 `BenchmarkIndexRebuild`、`BenchmarkIncrementalNoteUpdate`、`BenchmarkBacklinks`、`BenchmarkSearchLinkTarget`，运行 `go test ./internal/index -bench 'Benchmark(IndexRebuild|IncrementalNoteUpdate|Backlinks|SearchLinkTarget)' -benchmem`，退出码 0，输出基线：IndexRebuild 约 586ms/op、IncrementalNoteUpdate 约 51ms/op、Backlinks 约 0.57ms/op、SearchLinkTarget 约 36ms/op。首次 benchmark 暴露可选 note lookup 的 GORM `record not found` 日志；将 `UpdateNote` 和 `DeleteNote` 中允许 miss 的 lookup 改为 `Find + RowsAffected` 后重跑 benchmark，输出无该日志。随后运行 `go test ./internal/index -count=1` 和 `go test ./cmd/pinax -run 'IndexSearchDatabaseAndFiltersCLI|NoteLinkGraphCLI' -count=1`，均退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build` 和 `diff-impact`；结构发现实现影响集中在 `UpdateNote`、`DeleteNote` 两个增量入口及其 8 个调用方。
...
- [x] 4.2 将 doctor、repair plan 和 organize suggest 接入 link evidence。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 先新增 `cmd/pinax/main_test.go` 的 `TestDoctorRepairAndOrganizeUseLinkEvidence`，构造 broken `[[Missing Target]]`、ambiguous `[[Shared]]` 和无入出边 `Orphan.md`，并断言 `doctor` 输出 `broken_link`、`ambiguous_link`、`orphan_note`，`repair plan` 和 `organize suggest` 只生成 `manual_review` 的 `link_resolution`、`link_rewrite`、`orphan_review`，且默认不改写 Markdown、不创建 `.pinax/*-plans`。首次运行 `go test ./cmd/pinax -run 'DoctorRepairAndOrganizeUseLinkEvidence' -count=1`，退出码 1，失败于缺少 `broken_link` issue，确认旧逻辑未接入 link evidence。随后将 `buildVaultIssues` 和 `organizeFactOperations` 改为复用 `BuildEnhancedLinkGraph` / `ResolveLinkTarget`，新增 link evidence 透传和 `RepairOperation.target`、`PlanOperation.evidence`；重跑同一聚焦测试，退出码 0。运行验收命令 `go test ./internal/app ./cmd/pinax -run 'Doctor.*Link|Repair.*Link|Organize.*Link' -count=1`，退出码 0；补跑既有 doctor/repair/organize CLI 回归 `go test ./cmd/pinax -run 'VaultStatsDoctorAndDashboardCLI|RepairPlanJSONIsReadonlyAndSaveWritesPlanAsset|RepairApplyRequiresApprovalAndSnapshot|RepairApplyLowRiskOperationsAndRejectsStalePlan|OrganizeSuggestCreatesReviewableAgentPlan|OrganizeListEmptyGuidesSaveWorkflow' -count=1`，退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build` 和 `diff-impact`；结构发现 4.2 影响集中在 `internal/app/service.go` 的 doctor/repair/organize plan 生成路径，`internal/index` 仍只有 3.8 的 `UpdateNote`/`DeleteNote` 改动。

- [x] 4.3 扩展 dashboard 只读关系摘要。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 先新增 `internal/dashboard/server_test.go` 的 `TestReadonlyDashboardServesLinkGraphSummary`，覆盖 `/api/graph-summary` 返回 `dashboard.graph_summary` JSON、broken/ambiguous/orphans/engine facts、可运行 `pinax` next action，`/api/overview` 带 `link_graph` 摘要，首页 HTML 展示“关系/断链/歧义/孤立”，且 POST `/api/graph-summary` 返回 405。首次运行 `go test ./internal/dashboard -run 'LinkGraph|GraphSummary|Readonly' -count=1`，退出码 1，失败于 `/api/graph-summary` 落到首页 HTML，确认缺少只读 graph endpoint。实现新增 dashboard `handleGraphSummary`、`graphSummaryProjection`，并在 overview 和首页复用 `Service.GraphSummary`；重跑同一验收命令，退出码 0。随后运行 `go test ./internal/dashboard -count=1`，退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build` 和 `diff-impact`；结构发现 4.3 影响集中在 `internal/dashboard/server.go` 的 handler/projection 和 `internal/dashboard/server_test.go`。
...
- [x] 5.2 增强 `search --link-target` 查询。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 先新增 `cmd/pinax/main_test.go` 的 `TestSearchLinkTargetCLI` 和 `TestSearchLinkTargetAmbiguousCLI`，覆盖 `--link-target Beta`、`notes/beta.md`、`note_beta`、broken raw `Missing Target`、空白 filter `invalid_link_filter` 和同名标题 `link_target_ambiguous`。首次运行 `go test ./internal/index ./internal/app ./cmd/pinax -run 'Search.*LinkTarget|LinkTargetAmbiguous|InvalidLinkFilter' -count=1`，退出码 1，先暴露测试误把 JSON 中回显的 `filter.link_target=notes/beta.md` 当成结果 note；修正测试为解析 result note path 后重跑，退出码 1，真实失败为 `note_beta` 返回 0 且 ambiguous `Shared` 未报错。随后在 `SearchNotes` 中新增 `buildSearchLinkTargetFilter`，复用 `BuildEnhancedLinkGraph` 的 parser/resolver 结果计算 source paths，再统一过滤 index 与 fallback 搜索结果；空白 target 返回 `invalid_link_filter`，多候选 title 返回 `link_target_ambiguous` 且 hint 带候选路径，不自动选目标。重跑验收命令，退出码 0。补跑 `go test ./cmd/pinax -run 'IndexSearchDatabaseAndFiltersCLI|SearchLinkTarget|NoteLinkGraphCLI|NoteListFiltersCLI|SearchHelp' -count=1 && go test ./internal/app -run 'Search|NoteLinkGraph|GraphSummary' -count=1 && go test ./internal/index -run 'Search|LinkTarget' -count=1`，退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build`、`impact internal/app/service.go` 和 `diff-impact`；结构发现 5.2 影响集中在 app search projection/filter helper，CLI 入口仍只传递已有 `--link-target` flag。

- [x] 5.3 增加关系命令输出 contract tests。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 新增 `cmd/pinax/main_test.go` 的 `TestLinkOutputContractModes`、`TestBacklinkOutputContractAgent`、`TestOrphanOutputContractEvents`、`TestGraphExplainOutputContract`、`TestStdoutStderrLinkOutputContract`，覆盖 `note links/backlinks/orphans` 在 `--json`、`--agent`、`--events`、`--explain` 下的 envelope、facts、NDJSON start/end、错误 envelope、stdout/stderr 分离、无 ANSI/人类表格标签，并用含 `secret-token raw prompt system prompt` 的 note body 验证机器输出和 explain 不泄漏正文/secret。首次运行 `go test ./internal/output ./cmd/pinax -run 'LinkOutput|BacklinkOutput|OrphanOutput|GraphExplain|StdoutStderr' -count=1`，退出码 1，暴露 `note.links --json` 的 `data.note.body` 泄漏完整正文，以及断链 explain 缺少推荐下一步。随后将 link/backlink projection 的 `Data.note` 改为 `noteGraphNoteSummary`，并在 outgoing links 有 broken/ambiguous 时设置 `partial` 和 `repair_plan` action；重跑验收命令，退出码 0。补跑 `go test ./cmd/pinax -run 'NoteLinkGraphCLI|LinkOutput|BacklinkOutput|OrphanOutput|GraphExplain|StdoutStderr|SearchLinkTarget|NotebookCoreOutputContract' -count=1 && go test ./internal/app -run 'NoteLinks|NoteBacklinks|NoteOrphans|GraphSummary|Search' -count=1`，退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build` 和 `diff-impact`；结构发现 5.3 影响集中在 link graph projection 和 CLI contract tests。
...
- [x] 6.2 补低 token graph context 边界测试。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 先新增 `internal/mcpserver/server_test.go` 的 `TestGraphContextBounds` 和 `TestGraphContextTruncation`，构造 40 条出链大 fixture 和 8 个同名候选 fixture，断言 `pinax.note.context` 返回 `partial`、`facts.truncated=true`、edge 数量不超过 20、candidate 数量不超过 3、包含可运行 `pinax note links ... --json` next action，且不泄漏 note body 中的 `secret-token raw prompt hidden system prompt`。首次运行 `go test ./internal/mcpserver ./internal/app -run 'GraphContextBounds|GraphContextTruncation' -count=1`，退出码 1，失败于当前 MCP context 返回 `success` 且全量 40 条边、没有 truncation facts。随后在 `internal/mcpserver/server.go` 增加 `boundedGraphProjectionData`、`boundMCPNoteLinks` 和 `graphContextTruncationFacts`，限制 edges/candidates/evidence 并返回 `links.total/returned`、`backlinks.total/returned`、`candidates.truncated`、`evidence.truncated`、`truncated` facts；重跑验收命令，退出码 0。补跑 `go test ./internal/mcpserver -count=1` 和 `go test ./internal/mcpserver -run 'Graph|Links|Backlinks|Context|Readonly' -count=1`，均退出码 0。CodeGraph 使用目录 `/workspaces/yeisme-agent/cli/pinax`，运行过 `build` 和 `diff-impact`；结构发现 6.2 影响集中在 `Server.callTool` 的 `pinax.note.context` 只读响应边界和 MCP 测试。

## 7. E2E、证据和文档

- [x] 7.1 增加 testscript 双联 e2e。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 新增 `tests/e2e/bidirectional_links_test.go` 和 `tests/e2e/testdata/scripts/bidirectional_links.txt`，引入 `github.com/rogpeppe/go-internal/testscript`。测试先构建当前 `cmd/pinax` 到临时 PATH，再用 txtar fixture vault 跑真实 CLI：`note links Alpha` 在 scan fallback 下覆盖 wiki title、alias、heading、Markdown relative path、外部 URL ignored、同名歧义、断链；`note backlinks Beta` 覆盖反链；`note orphans` 覆盖孤立分类；`index rebuild` 后再次验证 fresh index facts；`search Alpha --link-target notes/delta.md` 覆盖 link-target；`search Alpha --link-target Shared` 断言 `link_target_ambiguous` 和候选路径。运行 `go test ./tests/e2e -run BidirectionalLinks -count=1`，退出码 0。该 e2e 只使用临时 fixture vault 和本地构建二进制，不依赖公网、真实 provider、用户 vault 或真实 token。

- [x] 7.2 增加 integration/e2e evidence 入口。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 新增 `internal/testkit/integrationevidence` Go runner 和 `Taskfile.yml` 的 `test:integration` 任务；runner 执行 `go test ./tests/e2e -run BidirectionalLinks -count=1`，生成 `temp/integration-test-runs/<run-id>/summary.json`、`command.txt`、`stdout.log`、`stderr.log`、`env.json` 和 `artifacts/README.txt`，并在测试失败时保留 evidence 后以原 exit code 退出。新增 `.gitignore` 条目 `temp/integration-test-runs/`，避免本地 evidence 进入 Git。运行 `task test:integration`，退出码 0，生成 run `temp/integration-test-runs/20260608T021607Z-646047`；随后运行 `find temp/integration-test-runs -maxdepth 3 -type f | sort`，看到 `summary.json`、`command.txt`、`stdout.log`、`stderr.log`、`env.json`、`artifacts/README.txt`。检查 `summary.json` 的 `schema_version` 为 `yeisme.integration_test_evidence.v1`、`exit_code=0`；运行 `rg -n -i 'secret|token|authorization|raw prompt|provider payload|hidden system prompt|private|chain-of-thought' <latest-run>`，无命中。

- [x] 7.3 更新 Pinax README 和 docs。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 更新 `README.md`、`docs/README.md`、`docs/product/mvp-scope.md`、`docs/interfaces/cli-output-contract.md`、`docs/operations/local-development.md` 和 `docs/architecture/architecture-boundaries.md`，补充 `pinax note links`、`pinax note backlinks`、`pinax note orphans`、`search --link-target`、broken/ambiguous/orphan 语义、index rebuild 建议、MCP 只读边界和 repair/organize manual review。运行 `rg -n "pinax note links|pinax note backlinks|pinax note orphans|--link-target|manual review|MCP" README.md docs`，退出码 0，命中 README 和 docs 相关入口；运行 `rg -n "index.*真源|SQLite.*真源|手写.*\.pinax|hand-write.*\.pinax" README.md docs || true`，退出码 0，只命中“不是笔记真源”“不能作为真源维护”“不要手写 `.pinax/*.json` 或 index metadata”等否定边界表述，没有把 SQLite index 描述成真源或要求手写 metadata。
...
- [x] 8.1 运行聚焦测试。
  ...
  - Evidence: 2026-06-08 运行 `go test ./internal/domain ./internal/index ./internal/app ./internal/output ./cmd/pinax ./internal/dashboard ./internal/mcpserver -run 'Link|Backlink|Orphan|Graph|Search.*Link|Repair.*Link|Organize.*Link|MCP' -count=1`，退出码 0，覆盖 domain、index、app、output、CLI、dashboard 和 MCP 关系相关测试；运行 `go test ./tests/e2e -run BidirectionalLinks -count=1`，退出码 0，双联 testscript/e2e 通过。

- [x] 8.2 运行本地质量门禁和 OpenSpec 校验。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 运行 `task check`，退出码 0；该任务执行 `gofmt` check、`openspec validate --all`、`go test ./...` 和 `go build -trimpath -ldflags="-s -w" -o dist/pinax ./cmd/pinax`，全量 19 个 OpenSpec 项通过、Go 测试和构建通过。运行 `openspec validate pinax-note-bidirectional-links --strict`，退出码 0，输出 `Change 'pinax-note-bidirectional-links' is valid`。运行 `openspec list`，退出码 0，显示 `pinax-note-bidirectional-links 25/28 tasks`，不是 `No tasks`。

- [x] 8.3 记录 closeout evidence 并准备归档。
  - Owner: `cli/pinax`
  ...
  - Evidence: 2026-06-08 归档前运行 `openspec validate pinax-note-bidirectional-links --strict`，退出码 0，输出 `Change 'pinax-note-bidirectional-links' is valid`。随后运行 `openspec archive pinax-note-bidirectional-links --yes`，退出码 0；命令提示 `Task status: 26/28 tasks` 和 2 个 checkbox 未勾，但继续归档并更新 specs：创建 `openspec/specs/note-bidirectional-links/spec.md`，更新 `note-command-ux`、`notebook-index-search` 和 `notebook-workflows` specs，归档到 `openspec/changes/archive/2026-06-08-pinax-note-bidirectional-links/`。未勾项是归档前 checklist 状态记录落后于已完成的 7.3 文档 evidence 和当前 8.3 closeout，本 archived tasks 已补正。归档后运行 `openspec validate --all`，退出码 0，19 个 OpenSpec 项全部通过。
