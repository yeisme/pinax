# pinax-agent-brain-layer Delta Spec
...
Pinax SHALL expose Agent Brain surfaces through MCP and Local REST/RPC as registered capabilities that are readonly by default and use explicit scope metadata for future team or HTTP deployments.
...
#### Scenario: Future HTTP and team surfaces require scope proof

- **WHEN** Agent Brain is exposed through HTTP MCP, OAuth, team workspace, or company knowledge mode
- **THEN** Pinax SHALL require explicit principal, workspace, source ACL, visibility, redaction policy, and audit reference semantics
- **AND** if scope proof is missing, Pinax SHALL return `permission_unknown`, `scope_required`, or an equivalent bounded failure rather than synthesizing cross-user knowledge.

#### Scenario: Team/company KB projection carries permission fields

- **WHEN** a future team/company Agent Brain projection includes evidence from more than one user, workspace, or organizational source
- **THEN** every claim or evidence group SHALL carry permission context including `principal`, `workspace`, `source_acl`, `visibility`, `redaction_policy`, and `audit_ref`
- **AND** `visibility=unknown` or missing `source_acl` SHALL prevent cross-user synthesis and return `permission_unknown`, `scope_required`, `insufficient_scope`, or `future_owner_required`
- **AND** local S3/rclone direct Cloud Sync transports SHALL NOT be treated as organization permission proof or rate limit proof.

#### Scenario: HTTP MCP and OAuth require future owner

- **WHEN** docs, discovery, or OpenAPI mention HTTP MCP, OAuth, team/company KB, or rate limit behavior
- **THEN** the capability SHALL be marked `planned` or `future-owner` unless an owning gateway/backend OpenSpec and implementation exist
- **AND** `cli/pinax` SHALL NOT expose fake HTTP routes, OAuth commands, hosted team commands, or production-ready team mode for unimplemented capabilities.

### Requirement: Ingest sources SHALL enter the vault through service-owned receipts

Pinax SHALL import or capture notes, meeting records, email exports, document bundles, webhook payloads, shortcuts, and other future sources through service-owned commands and receipts before they become searchable brain context.
...
#### Scenario: Future source connectors do not become truth sources

- **WHEN** future connectors import email, calendar, Slack, Feishu, CRM, Twitter, voice, webhook, Zapier, IFTTT, or Apple Shortcuts content
- **THEN** Pinax SHALL treat those systems as ingest providers or delivery surfaces
- **AND** the Markdown vault and service-owned receipts SHALL remain the local source of truth for Agent Brain recall.
...
Future Web, desktop, mobile, or hosted clients SHALL consume Pinax Agent Brain projections rather than duplicating vault parsing, graph construction, memory ranking, provider calls, or proof-loop business rules.
...
#### Scenario: Hosted/team client has separate owner

- **WHEN** a future hosted, OAuth, multi-user, or company knowledge client is built
- **THEN** it SHALL be owned by a separate client/backend/gateway OpenSpec
- **AND** `cli/pinax` SHALL remain responsible for local CLI/API/projection contracts, permission gates, redaction, and proof-loop behavior.
