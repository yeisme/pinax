...
- [ ] 2.1 为 `internal/templateengine` 写 Go template 失败测试。
  - Owner: `cli/pinax`
...
- [ ] 2.2 实现 `internal/templateengine` 核心。
  - Owner: `cli/pinax`
...
- [ ] 2.3 增加 legacy simple 模板兼容测试和实现。
  - Owner: `cli/pinax`
...
- [ ] 3.1 为 v2 frontmatter parser 写失败测试。
  - Owner: `cli/pinax`
...
- [ ] 3.3 将 `RenderTemplate` 和 `CreateNote --template` 切到共享引擎。
  - Owner: `cli/pinax`
...
- [ ] 4.1 为 query declaration 和 fenced query block 写失败测试。
  - Owner: `cli/pinax`
...
- [ ] 4.2 实现 query declaration 解析和 inspect explain。
  - Owner: `cli/pinax`
...
- [ ] 4.3 实现 query-backed render context 注入。
  - Owner: `cli/pinax`
...
- [ ] 4.4 增加 query result 渲染 helper 和安全测试。
  - Owner: `cli/pinax`
...
- [ ] 4.5 增加 query template CLI workflow 测试。
  - Owner: `cli/pinax`
...
- [ ] 5.1 增加 `template inspect` 和 `template preview` CLI 测试。
  - Owner: `cli/pinax`
...
- [ ] 5.2 Wire `template inspect`、`template preview` 和 `--engine`。
  - Owner: `cli/pinax`
...
- [ ] 5.4 增强 `pinax note show` 渲染查看命令测试。
  - Owner: `cli/pinax`
...
- [ ] 5.5 增强 `pinax note refresh` 写回受控区块测试和实现计划。
  - Owner: `cli/pinax`
...
- [ ] 5.6 增加镜像 note/template 路径的渲染版本、长参数复用和 snapshot 测试。
  - Owner: `cli/pinax`
...
- [ ] 5.7 增加 render run 检索、alias 解析和 Tab completion 测试。
  - Owner: `cli/pinax`
...
- [ ] 5.8 增加 render run prune/repair 维护命令计划和测试。
  - Owner: `cli/pinax`
