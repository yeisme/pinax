...
- [ ] 0.1 确认本 change 分三类交付：先做 CLI 使用辅助，再做懒加载启动检索，最后做 typed property database / Pinax SQL。
...
- [ ] 1.1 增加 completion fixture 和命令级测试。
...
- [ ] 2.1 定义 lazy index policy 和请求字段。
...
- [ ] 2.2 为首次 search lazy rebuild 写回归测试。
...
- [ ] 2.4 设计 database/SQL query 的显式 lazy index 行为。
...
- [ ] 3.1 增加 `database view` 命令测试，先兼容现有 filter-only saved views。
...
- [ ] 3.2 实现 database view 命令 wiring。
...
- [ ] 3.3 升级 `.pinax/views.json` 读取兼容策略设计。
...
- [ ] 4.1 新增 database view fixture vault。
...
- [ ] 5.1 新增 Pinax SQL parser golden tests。
...
- [ ] 5.2 实现 Pinax SQL lexer/parser 和 semantic validator。
...
- [ ] 6.1 实现 `query run` table/list/task result projection。
...
- [ ] 6.2 实现 cursor pagination 和 selected property loading。
...
- [ ] 6.3 支持 `note list --property` 和 `--strict-properties`。
...
- [ ] 7.3 实现 v2 database view registry。
...
- [ ] 8.2 增加只读 MCP query/view surface。
