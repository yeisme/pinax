# vault-dashboard-health Specification

## Purpose

描述 Pinax 本地 Markdown vault 的统计、健康检查和只读 dashboard 能力。
## Requirements
...
- **WHEN** a user runs `pinax vault stats --vault ./my-notes`
...
- **WHEN** a user runs `pinax vault stats --vault ./my-notes --json`
...
- **THEN** `pinax vault stats --vault ./my-notes --json` SHALL still compute Markdown scan metrics
- **AND** the projection SHALL report `index_status=missing` with a runnable `pinax index rebuild` next action.

#### Scenario: Root stats alias remains compatible
- **WHEN** a user runs `pinax stats --vault ./my-notes --json`
- **THEN** Pinax SHALL preserve the existing machine output contract
- **AND** the root alias MAY be hidden from primary help.
...
- **WHEN** a user runs `pinax vault doctor --vault ./my-notes --json`
...
- **WHEN** a user runs `pinax vault doctor --vault ./my-notes`
...
- **WHEN** a user runs `pinax vault doctor --vault ./my-notes --agent`
...
#### Scenario: Root doctor alias remains compatible
...
- **WHEN** a user runs `pinax vault dashboard --vault ./my-notes --port 0`
...
- **THEN** it SHALL call the same application services used by `pinax vault stats` and `pinax vault doctor`
...
#### Scenario: Root dashboard alias remains compatible
- **WHEN** a user runs `pinax dashboard --vault ./my-notes --port 0`
- **THEN** Pinax SHALL preserve backwards-compatible dashboard startup behavior
- **AND** the root alias MAY be hidden from primary help.
...
- **WHEN** `pinax vault stats`, `pinax vault doctor`, or dashboard data export is run with `--json` or `--agent`
