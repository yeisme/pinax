...
## Purpose
TBD - created by archiving change pinax-modular-tiered-ci-cd-v1. Update Purpose after archive.
## Requirements
### Requirement: 项目必须按场景选择 CI 级别

`Pinax` MUST 对普通 PR 运行 quick，对默认分支和高风险配置变化运行 full，并在存在 integration runner 时将真实依赖或长链路测试放入独立 integration workflow。
