...
## Yeisme OpenSpec Architecture Handoff Gate

If this architecture or engineering review produces an accepted formal delivery方案, implementation approach, test strategy, migration path, or handoff, do not leave it only in the review summary or plan file. Before durable implementation, release, archive, parallel handoff, or multi-session execution, create or update the correct OpenSpec change. Bounded exploratory MVPs, prototypes, spikes, UI/UX extensions, local refactors, and focused bug investigations may run first if they are reversible, locally verified, and do not touch stable contracts, persistence schemas, cross-project boundaries, production behavior, credentials, or external side effects:

- Root `openspec/changes/<design-id>/` for repository-level architecture, governance, migration, and cross-project handoff.
...
- `proposal.md`, `design.md`, `tasks.md`, and `specs/**/spec.md` in Chinese by default for human-facing content.
- At least one Mermaid diagram in `design.md` for architecture, data flow, state, dependency, sequence, or rollback.
- Atomic `tasks.md` items with owner, scope, dependencies, parallel lane, acceptance criteria, validation command or manual smoke, expected result, and failure re-check.
- A parallelization section that either defines lanes that can run in separate worktrees or explains why the work must stay sequential.
- Task requirements for Chinese comments in new or changed complex logic, state machines, concurrency, error handling, boundary checks, protocol conversions, and non-obvious test fixtures.

If the review cannot write files in the current mode, explicitly output the required OpenSpec owner path and state that formal delivery should not start until the OpenSpec tasks exist. Exploratory work may continue only within the bounded MVP/prototype/spike rules above.
