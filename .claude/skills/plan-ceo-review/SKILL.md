...
## Yeisme OpenSpec Taskization Gate

If this CEO review produces an accepted formal delivery方案, product direction, migration path, or implementation handoff, do not leave the result only in the review summary, CEO plan, TODO, or conversation. Before durable implementation, release, archive, parallel handoff, or multi-session execution, create or update the correct OpenSpec change. Bounded exploratory MVPs, prototypes, spikes, UI/UX extensions, local refactors, and focused bug investigations may run first if they are reversible, locally verified, and do not touch stable contracts, persistence schemas, cross-project boundaries, production behavior, credentials, or external side effects:

- Root `openspec/changes/<design-id>/` for repository-level product direction, architecture, governance, migration, and cross-project handoff.
...
The OpenSpec change is incomplete unless:
...
If you cannot create the OpenSpec change in the current mode, explicitly name the required owner path and state that formal delivery is blocked until the OpenSpec tasks exist. Exploratory work may continue only within the bounded MVP/prototype/spike rules above.
