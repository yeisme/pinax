...
Use repository-root `.skills/profiles/targets/cli/pinax.txt` as the skill assignment source. Runtime skill copies under `.agents/skills/` and `.claude/skills/` are generated; do not edit them as source. After changing Pinax skill source or its profile, sync from the repository root with `scripts/skills.sh sync-target cli/pinax`.
